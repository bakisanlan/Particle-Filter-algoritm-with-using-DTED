...
%% PF Functions
    methods
        function self = ParticleFilter(N,x_range,y_range,hdg_range,std,dt,dted,alt)
            self.N = N;
            ...
            self.create_uniform_particles(self);
        end
        ...
        function aircraft_pos = aircraft_step(self,aircraft_pos, u)
            ...
            % Update heading
            aircraft_pos(3) = u(1) + (randn(1) * self.std(1));
            aircraft_pos(3) = mod(aircraft_pos(3), 2*pi);
            
            % Move in the (noisy) commanded direction
            velocity = (u(2) * self.dt) + (randn(1) * self.std(2));
        ...
        function particle_step(self, u)
            ...
            % Update heading
            self.particles(:, 3) = u(1) + (randn(self.N, 1) .* self.std(1));
            self.particles(:, 3) = mod(self.particles(:, 3), 2*pi);
            
            % Move in the (noisy) commanded direction
            velocity = (u(2) * self.dt) + (randn(self.N, 1) * self.std(2));
        ...
        function update_weights(self, radar_data)
            % update weight based on PDF value

            % updating elevation data of particles
            self.find_elev_particles(self);

            self.weights = self.weights .* normpdf(radar_data,self.particles_elevation,self.std);
            self.weights = self.weights + 1e-300;
            self.weights = self.weights ./ sum(self.weights);

            if self.neff(self) < self.N/2
                indexes = self.resample_Systematic(self);
                self.resample(self,indexes)
            end
        end
        ...
        function estimate(self)
            ...
            self.mean = sum(pos .* self.weights) / sum(self.weights);
            self.var  = sum((pos - self.mean).^2 .* self.weights) / sum(self.weights);
        end
...
%% Functions that is used in Class as subfunctions
    methods
        ...
        function find_elev_particles(self)
            % find elevation of particles from DTED
    
            self.particles_elevation = interp2(self.dted{1},self.dted{2},self.dted{3},self.particles(:,1),self.particles(:,2));
            self.particles_elevation = self.alt - self.particles_elevation + randn(self.N,1)*self.std;
        end
