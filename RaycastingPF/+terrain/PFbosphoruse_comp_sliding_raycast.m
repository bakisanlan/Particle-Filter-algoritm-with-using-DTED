...
psi0    = 20*pi/180;
Ts      = 2;
hAircraft.Pose          = [5*x0-700; 6*y0-1000; z0; psi0];
phi_r = 70;
...
hEstimator.particles = [5*x0-700 6*y0-600 z0 psi0];
...
while simTime < Tf
    ...
    radar_pt_p = plot3(radar_pt_lla(:,2),radar_pt_lla(:,1),radar_pt_lla(:,3),'c.','MarkerSize',20,'DisplayName', 'Aircraft Radar PointC');
    particle_pt_p_ray = plot3(particle_pt_lla(:,2),particle_pt_lla(:,1),particle_pt_lla(:,3),'m.','MarkerSize',20,'DisplayName','Particle Sliding MF PointC');

    uav_pos_lla = ned2lla([hAircraft.Pose(2) hAircraft.Pose(1) -hAircraft.Pose(3)],[left_lower 0],"flat");
    part_pos_lla = ned2lla([hEstimator.particles(iPart,2) hEstimator.particles(iPart,1) -hAircraft.Pose(3)],[left_lower 0],"flat");

    % for k=1:length(radar_pt_lla)
    %     plot3([uav_pos_lla(2) radar_pt_lla(k,2)],[uav_pos_lla(1) radar_pt_lla(k,1)], [uav_pos_lla(3) radar_pt_lla(k,3)],'c')
    %     hold on
    % end
    % 
    % for k=1:length(particle_pt_lla)
    %     plot3([part_pos_lla(2) particle_pt_lla(k,2)],[part_pos_lla(1) particle_pt_lla(k,1)], [hAircraft.Pose(3) particle_pt_lla(k,3)],'m')
    %     hold on
    % end

    X_uav2radarPC = [repmat(uav_pos_lla(2),size(radar_pt_lla, 1), 1), radar_pt_lla(:,2)]';
    Y_uav2radarPC = [repmat(uav_pos_lla(1),size(radar_pt_lla, 1), 1), radar_pt_lla(:,1)]';
    Z_uav2radarPC = [repmat(uav_pos_lla(3),size(radar_pt_lla, 1), 1), radar_pt_lla(:,3)]';

    h = plot3(X_uav2radarPC,Y_uav2radarPC,Z_uav2radarPC,'c-');
    set(h, 'HandleVisibility', 'off'); % Hide individual lines from legend
    plot3(NaN, NaN, NaN, 'c-', 'DisplayName', 'Aircraft Radar Rays');

    X_part2MFPC = [repmat(part_pos_lla(2),size(particle_pt_lla, 1), 1), particle_pt_lla(:,2)]';
    Y_part2MFPC = [repmat(part_pos_lla(1),size(particle_pt_lla, 1), 1), particle_pt_lla(:,1)]';
    Z_part2MFPC = [repmat(part_pos_lla(3),size(particle_pt_lla, 1), 1), particle_pt_lla(:,3)]';

    h = plot3(X_part2MFPC,Y_part2MFPC,Z_part2MFPC,'m-');
    set(h, 'HandleVisibility', 'off'); % Hide individual lines from legend
    plot3(NaN, NaN, NaN, 'm-', 'DisplayName', 'Particle Sliding MF Rays');
